<template>
  <el-dialog v-model="visible" :title="$t('t_offline_transfer_set')" width="30%" destroy-on-close>
    <div class="reply-box">
      <span class="reply-title">{{ $t('t_reply_set_timeout_span') }}</span>

      <!--      设置超时时间-->
      <div class="set-reply-title flex row">
        <span>{{ $t('t_offline_transfer_set_time') }}</span>
        <el-input v-model="replyValue" style="width: 4rem" />
        <SelectComponent @set-value="setEmitValue" :init-value="props.unit" />
      </div>

      <div style="margin-top: 2rem">
        <span class="reply-title" style="margin-left: 100px"
          >{{ $t('t_offline_transfer_set_desc') }}}</span
        >
      </div>
    </div>
...
import { TimeUnit, updateSessionAutoTransfer } from '@/server/system_config_request'
...
const sureConfirm = () => {
  ...
  updateSessionAutoTransfer(param).then((res) => {
    if (res && res.code == '0') {
