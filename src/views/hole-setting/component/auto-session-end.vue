<template>
  <el-dialog v-model="visible" :title="$t('t_disconnect_session_set')" width="30%" destroy-on-close>
    <div class="reply-box">
      <span class="reply-title">{{ $t('t_reply_set_timeout_span') }}</span>

      <!--      设置超时时间-->
      <div class="set-reply-title flex row">
        <span>{{ $t('t_disconnect_time') }}</span>
      ...
      <div style="margin-top: 2rem">
        <span class="reply-title" style="margin-left: 100px"
          >{{ $t('t_disconnect_session_set_desc') }}}</span
        >
      </div>
    </div>
...
import { TimeUnit, updateSessionAutoEnd } from '@/server/system_config_request'
...
const sureConfirm = () => {
  ...
  updateSessionAutoEnd(param).then((res) => {
    if (res && res.code == '0') {
